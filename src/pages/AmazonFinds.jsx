...
export default function AmazonFinds() {
  ...
  const amazonItems = products.filter(
    (item) =>
      item.amazon_url ||
      item.affiliate_url ||
      item.category?.toLowerCase() === 'amazon finds' ||
      item.category_slug === 'amazon-finds'
  );
  ...
  return (
    <Layout>
      <Seo
        title="Amazon Finds"
        description="Explore top-rated Amazon Finds across beauty, fashion, lifestyle, and decor curated for value and style."
      />
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <SectionTitle
          eyebrow="Amazon Finds"
          title="Curated budget-friendly picks"
          description="Handpicked recommendations directly from Amazon."
        />
        ...
        {isLoading ? (
          ...
          <div className="rounded-[32px] border border-[#e9d5ff]/80 bg-white/90 p-12 text-center shadow-xl shadow-purple-950/5 backdrop-blur-xl">
            ...
            <h3 className="mt-2 text-2xl font-bold text-[#2e1f3b]">
              No published Amazon finds found
            </h3>
            <p className="mx-auto mt-2 max-w-md text-sm text-[#2e1f3b]/75">
              Check back soon for new Amazon recommendations!
            </p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {amazonItems.map((product) => (
