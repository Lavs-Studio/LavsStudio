...
export default function SkinCare() {
  ...
  const skinCareItems = products.filter(
    (item) =>
      item.category?.toLowerCase() === 'skin care' ||
  ...
  return (
    <Layout>
      <Seo
        ...
        description="Gentle hydration, Korean beauty favourites, and glowing skin recommendations curated by Lavs Studio."
      />
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <SectionTitle
          eyebrow="Skincare"
          title="Calm, hydrating routines"
          description="Build a gentle routine for glowing, comfortable skin."
        />
        ...
        {isLoading ? (
          ...
          <div className="rounded-[32px] border border-[#e9d5ff]/80 bg-white/90 p-12 text-center shadow-xl shadow-purple-950/5 backdrop-blur-xl">
            ...
            <h3 className="mt-2 text-2xl font-bold text-[#2e1f3b]">
              No published skincare items found
            </h3>
            <p className="mx-auto mt-2 max-w-md text-sm text-[#2e1f3b]/75">
              Check back soon for new skincare routines and recommendations!
            </p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
